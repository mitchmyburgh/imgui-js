...
const NavWinHighlightDelay = 0.2; // time before highlight an and screen dimming
...
export class NavMoveResult {
  ...
  DistAxial: number = Number.MAX_VALUE;
  RectRel: any = null; // todo type
...
export var ImguiNavMixin = {
  isNavInputPressed(n, mode) {
    return this.getNavInputAmount(n, mode) > 0;
  },

  isNavInputPressedAnyOfTwo(n1, n2, mode) {
    let sum =
      this.getNavInputAmount(n1, mode) + this.getNavInputAmount(n2, mode);
    return sum > 0;
  },
  ...
  navUpdate() {
    ...
    let nav_keyboard_active =
      (g.IO.ConfigFlags & ConfigFlags.NavEnableKeyboard) != 0;
    let nav_gamepad_active =
      (g.IO.ConfigFlags & ConfigFlags.NavEnableGamepad) != 0 &&
      (g.IO.BackendFlags & BackendFlags.HasGamepad) != 0;
    if (nav_gamepad_active) {
      if (
        g.IO.NavInputs[NavInput.Activate] > 0 ||
    ...
    if (nav_keyboard_active) {
      ...
      if (g.IO.KeyCtrl) g.IO.NavInputs[NavInput.TweakSlow] = 1;
      if (g.IO.KeyShift) g.IO.NavInputs[NavInput.TweakFast] = 1;
    ...
    for (let i = 0; i < g.IO.NavInputs.length; i++) {
      g.IO.NavInputsDownDuration[i] =
        g.IO.NavInputs[i] > 0
          ? g.IO.NavInputsDownDuration[i] < 0
            ? 0
    ...
    if (
      ...
      if (g.NavInitRequestFromMove)
        this.setNavIDWithRectRel(
          g.NavInitResultId,
          g.NavLayer,
          g.NavInitResultRectRel
        );
      else this.setNavID(g.NavInitResultId, g.NavLayer);
    ...
    if (g.NavMoveRequest) this.navUpdateMoveResult();
    ...
    if (g.NavWindow) this.navSaveLastChildNavWindowIntoParent(g.NavWindow);
    if (
      g.NavWindow &&
    ...
    g.IO.NavActive =
      (nav_keyboard_active || nav_gamepad_active) &&
      g.NavWindow &&
    ...
    if (this.isNavInputPressed(NavInput.Cancel, InputReadMode.Pressed)) {
      ...
      } else if (g.OpenPopupStack.length > 0) {
    ...
    if (g.NavMoveRequestForward == NavForward.None) {
      ...
      if (
        ...
        if (
          allowed_dir_flags & (1 << Dir.Left) &&
          this.isNavInputPressedAnyOfTwo(
            NavInput.DpadLeft,
            NavInput.KeyLeft_,
    ...
    if (nav_keyboard_active)
      nav_scoring_rect_offset_y =
        this.navUpdatePageUpPageDown(allowed_dir_flags);
    ...
    if (
      ...
      let scroll_dir = this.getNavInputAmount2d(
      ...
      if (scroll_dir.x != 0 && win.ScrollbarX) {
        win.SetWindowScrollX(
          Math.floor(win.Scroll.x + scroll_dir.x * scroll_speed)
        );
        g.NavMoveFromClampedRefRect = true;
      }
      if (scroll_dir.y != 0) {
        win.SetWindowScrollY(
          Math.floor(win.Scroll.y + scroll_dir.y * scroll_speed)
        );
        g.NavMoveFromClampedRefRect = true;
      }
    }
    ...
    if (g.NavMoveRequest && g.NavMoveFromClampedRefRect && g.NavLayer == 0) {
      ...
      if (!window_rect_rel.Contains(win.NavRectRel[g.NavLayer])) {
        ...
        let pad = win.CalcLineHeight() * 0.5;
        window_rect_rel.expandXY(
          -Math.min(window_rect_rel.GetWidth(), pad),
    ...
    else nav_rect_rel = Rect.FromXY(0, 0, 0, 0);
    if (g.NavWindow) {
    ...
    g.NavScoringRectScreen.Min.x = Math.min(
      g.NavScoringRectScreen.Min.x + 1,
  ...
  navProcessItem(win, nav_bb, id) {
    ...
    let nav_bb_rel = new Rect(
      Vec2.Subtract(nav_bb.Min, win.Pos),
    ...
    if (
      ...
      if (
        g.NavMoveRequestFlags & NavMoveFlags.AlsoScoreVisibleSet &&
        win.ClipRect.Overlaps(nav_bb)
      ) {
        if (
          Vec1.Clamp(nav_bb.Max.y, win.ClipRect.Min.y, win.ClipRect.Max.y) -
  ...
  getNavInputAmount2d(dir_sources, mode, slow_factor, fast_factor) {
    ...
    if (dir_sources & NavDirSourceFlags.Keyboard) {
      delta.AddXY(
        this.getNavInputAmount(NavInput.KeyRight_, mode) -
    ...
    if (dir_sources & NavDirSourceFlags.PadDPad) {
      delta.AddXY(
        this.getNavInputAmount(NavInput.DpadRight, mode) -
    ...
    if (dir_sources & NavDirSourceFlags.PadLStick) {
      delta.AddXY(
        this.getNavInputAmount(NavInput.LStickRight, mode) -
  ...
  activateItem(
    id // remotely activate a button, checkbox, ..
  ) {
  ...
  navUpdateWindowing() {
    ...
    let start_windowing_with_gamepad =
      !g.NavWindowingTarget &&
      this.isNavInputPressed(NavInput.Menu, InputReadMode.Pressed);
    let start_windowing_with_keyboard =
      !g.NavWindowingTarget &&
      g.IO.KeyCtrl &&
    ...
    if (start_windowing_with_gamepad || start_windowing_with_keyboard) {
      let window = g.NavWindow
        ? g.NavWindow
        : this.findWindowNavFocusable(
    ...
    if (g.NavWindowingTarget && g.NavInputSource == InputSource.NavGamepad) {
      ...
      const focus_change_dir =
        Math.floor(
          this.isNavInputPressed(NavInput.FocusPrev, InputReadMode.RepeatSlow)
        ) -
        Math.floor(
          this.isNavInputPressed(NavInput.FocusNext, InputReadMode.RepeatSlow)
        );
    ...
    if (
      ...
      g.NavDisableMouseHover = true;
      apply_focus_window =
        this.navRestoreLastChildNavWindow(apply_focus_window);
  ...
  navUpdateWindowingList() {
    ...
    this.SetNextWindowPos(
      Vec2.Mult(g.IO.DisplaySize, 0.5),
      CondFlags.Always,
      new Vec2(0.5, 0.5)
    );
    ...
    for (let n = g.WindowsFocusOrder.length - 1; n >= 0; n--) {
      ...
      if (!this.isWindowNavFocusable(window)) continue;
  ...
  navCalcPreferredRefPos() {
    ...
    if (g.NavDisableHighlight || !g.NavDisableMouseHover || !g.NavWindow) {
      ...
      if (this.IsMousePosValid(g.IO.MousePos)) return g.IO.MousePos;
      ...
      let pos = Vec2.AddXY(
        g.NavWindow.Pos,
        rect_rel.Min.x +
          Math.min(g.Style.FramePadding.x * 4, rect_rel.GetWidth()),
        rect_rel.Max.y - Math.min(g.Style.FramePadding.y, rect_rel.GetHeight())
      );
  ...
  navUpdateMoveResult() {
    ...
    if (g.NavLayer == 0) {
      ...
      let next_scroll =
        result.Window.calcNextScrollFromScrollTargetAndClamp(false);
      ...
      if (result.Window.Flags & WindowFlags.ChildWindow)
        this.navScrollToBringItemIntoView(
          result.Window.ParentWindow,
          new Rect(
            Vec2.Add(rect_abs.Min, delta_scroll),
  ...
  navScrollToBringItemIntoView(win, item_rect) {
    let window_rect = new Rect(
      Vec2.AddXY(win.InnerMainRect.Min, -1, -1),
    ...
    if (win.ScrollbarX && item_rect.Min.x < window_rect.Min.x) {
      win.ScrollTarget.x =
        item_rect.Min.x - win.Pos.x + win.Scroll.x - g.Style.ItemSpacing.x;
      win.ScrollTargetCenterRatio.x = 0;
    } else if (win.ScrollbarX && item_rect.Max.x >= window_rect.Max.x) {
      win.ScrollTarget.x =
        item_rect.Max.x - win.Pos.x + win.Scroll.x + g.Style.ItemSpacing.x;
      win.ScrollTargetCenterRatio.x = 1;
    }
    if (item_rect.Min.y < window_rect.Min.y) {
      win.ScrollTarget.y =
        item_rect.Min.y - win.Pos.y + win.Scroll.y - g.Style.ItemSpacing.y;
      win.ScrollTargetCenterRatio.y = 0;
    } else if (item_rect.Max.y >= window_rect.Max.y) {
      win.ScrollTarget.y =
        item_rect.Max.y - win.Pos.y + win.Scroll.y + g.Style.ItemSpacing.y;
  ...
  navUpdatePageUpPageDown(allowed_dir_flags) {
    ...
    if (
      ...
      let win = g.NavWindow;
      let page_up_held =
        this.IsKeyDown(g.IO.KeyMap[Key.PageUp]) &&
        allowed_dir_flags & (1 << Dir.Up);
      let page_down_held =
        this.IsKeyDown(g.IO.KeyMap[Key.PageDown]) &&
        allowed_dir_flags & (1 << Dir.Down);
      if (page_up_held != page_down_held) {
        ...
        if (win.DC.NavLayerActiveMask == 0x00 && win.DC.NavHasScroll) {
          ...
          else if (this.IsKeyPressed(g.IO.KeyMap[Key.PageDown], true))
            win.SetWindowScrollY(win.Scroll.y + win.InnerClipRect.GetHeight());
        } else {
          let nav_rect_rel = win.NavRectRel[g.NavLayer];
          const page_offset_y = Math.max(
            0,
          ...
          if (this.IsKeyPressed(g.IO.KeyMap[Key.PageUp], true)) {
            ...
            g.NavMoveRequestFlags =
              NavMoveFlags.AllowCurrentNavId | NavMoveFlags.AlsoScoreVisibleSet;
          } else if (this.IsKeyPressed(g.IO.KeyMap[Key.PageDown], true)) {
            nav_scoring_rect_offset_y = page_offset_y;
            // Because our scoring rect is offset, we intentionally
            // request the opposite direction (so we can always land
  ...
  navUpdateAnyRequestFlag() {
    ...
    if (g.NavAnyRequest) console.assert(g.NavWindow);
  },
  ...
  navGetDirQuadrantFromDelta(dx, dy) {
    ...
    else return dy > 0 ? Dir.Down : Dir.Up;
  },
  ...
  navScoreItem(result, cand) {
    ...
    if (g.NavLayer != win.DC.NavLayerCurrent) return false;
    ...
    if (win.ParentWindow == g.NavWindow) {
      ...
      if (!win.ClipRect.Contains(cand)) return false;
    ...
    this.navClampRectToVisibleAreaForMoveDir(
      g.NavMoveClipDir,
      cand,
      win.ClipRect
    );
    ...
    let dbx = this.navScoreItemDistInterval(
      cand.Min.x,
      cand.Max.x,
      curr.Min.x,
      curr.Max.x
    ...
    let dby = this.navScoreItemDistInterval(
      Vec1.Lerp(cand.Min.y, cand.Max.y, 0.2),
      Vec1.Lerp(cand.Min.y, cand.Max.y, 0.8),
      Vec1.Lerp(curr.Min.y, curr.Max.y, 0.2),
      Vec1.Lerp(curr.Min.y, curr.Max.y, 0.8)
    );
    if (dby != 0 && dbx != 0) dbx = dbx / 1000 + (dbx > 0 ? +1 : -1);
