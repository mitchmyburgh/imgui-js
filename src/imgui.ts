...
export class Imgui extends ImguiMixins {
  ...
  NewFrame(time) {
    ...
    if (this.debug) {
      ...
      console.assert(
        g.Style.WindowMinSize.x >= 1 && g.Style.WindowMinSize.y >= 1
      ); // Invalid style setting.
      for (let n = 0; n < g.IO.KeyMap.length; n++)
        console.assert(
          g.IO.KeyMap[n] >= -1 && g.IO.KeyMap[n] < g.IO.KeysDown.length
    ...
    if (
      ...
      !(g.IO.BackendFlags & BackendFlags.HasMouseCursors)
    )
    ...
    if (g.SettingsDirtyTimer > 0) {
      ...
      if (g.SettingsDirtyTimer <= 0) {
        if (g.IO.IniFilename) this.SaveIniSettingsToDisk(g.IO.IniFilename);
        else {
    ...
    g.FramerateSecPerFrameIdx =
      (g.FramerateSecPerFrameIdx + 1) % g.FramerateSecPerFrame.length;
    g.IO.Framerate =
      g.FramerateSecPerFrameAccum > 0
    ...
    if (g.ActiveId == 0 && g.FocusTabPressed) {
      ...
      else g.FocusRequestNextCounterTab = g.IO.KeyShift ? -1 : 0;
    }
    ...
    if (g.FocusRequestNextWindow != null) {
      ...
      if (
        ...
        g.FocusRequestCurrCounterAll = this.modPositive(
      ...
      if (
        ...
        g.FocusRequestCurrCounterTab = this.modPositive(
          g.FocusRequestNextCounterTab,
          win.DC.FocusCounterTab + 1
        );
      }
  ...
  EndFrame() {
    ...
    if (g.NavWindowingTarget) this.NavUpdateWindowingList();
    ...
    if (g.DragDropActive) {
      ...
      if (is_delivered || is_elapsed) this.ClearDragDrop();
    }
    ...
    for (let i = 0; i != g.Windows.length; i++) {
      ...
      if (win.Active && win.Flags & WindowFlags.ChildWindow) continue;
  ...
  Render() {
    let g = this.guictx;
    console.assert(g.Initialized);
    if (g.FrameCountEnded != g.FrameCount) this.EndFrame();
    ...
    if (g.NavWindowingTarget) frontWins[1] = g.NavWindowingList;
  ...
  PushStyleColor(
    stylid,
    col // stylid is a string like "WindowBg"
  ) {
    let g = this.guictx;
  ...
  SetFont(
    font // generally prefer Push/PopFont
  ) {
    let g = this.guictx;
    g.Font = font;
  ...
  GetFontSize() {
    // useful for computing horizontal positioning (aka 'em')
    let g = this.guictx;
    return g.FontSize;
  }
  ...
  GetLineHeight() {
    // useful for computing vertical spacing
    let g = this.guictx;
  ...
  pushMultiItemsWidths(components, w_full = 0) {
    ...
    if (w_full <= 0) w_full = this.CalcItemWidth();
  ...
  PushItemFlag(flag, val) {
    let win = this.guictx.CurrentWindow;
    if (val) win.DC.ItemFlags |= flag;
    else win.DC.ItemFlags &= ~flag;
    ...
    if (g.NavWindow) {
      ...
      if (froot) {
        if (froot.WasActive && froot != win.RootWindow) {
          ...
          if (froot.Flags & WindowFlags.Modal) return false;
          if (
            froot.Flags & WindowFlags.Popup &&
  ...
  SetItemDefaultFocus() {
    ...
    if (
      ...
      if (!this.IsItemVisible()) this.SetScrollHereY();
    }
  }
  ...
  IsItemHovered(flags = 0, delay = 0) {
    ...
    //    return false;
    if (
      g.HoveredRootWindow != win.RootWindow &&
    ...
    if (!this.isWindowContentHoverable(win, flags)) return false;

    // Test if the item is disabled
    if (
      win.DC.ItemFlags & ItemFlags.Disabled &&
  ...
  IsRectVisible(a, b) {
    ...
    if (b == undefined) {
      ...
      return win.ClipRect.Overlaps(
        new Rect(win.DC.CursorPos, Vec2.Add(win.DC.CursorPos, a))
      );
    } else {
  ...
  IsKeyDown(user_key_index) {
    ...
    console.assert(
      user_key_index >= 0 && user_key_index < this.guictx.IO.KeysDown.length
    );
  ...
  IsKeyReleased(user_key_index) {
    ...
    return (
      g.IO.KeysDownDurationPrev[user_key_index] >= 0 &&
  ...
  IsMouseDragging(button = 0, lock_threshold = -1) {
    ...
    if (lock_threshold < 0) lock_threshold = g.IO.MouseDragThreshold;
    return (
  ...
  IsMouseHoveringRect(r_min, r_max, clip = true) {
    ...
    const rect_for_touch = new Rect(
      Vec2.Subtract(rect_clipped.Min, g.Style.TouchExtraPadding),
      Vec2.Add(rect_clipped.Max, g.Style.TouchExtraPadding)
    );
  ...
  GetMouseDragDelta(button = 0, lock_threshold = -1) {
    ...
    if (g.IO.MouseDown[button] || g.IO.MouseReleased[button]) {
      if (
        g.IO.MouseDragMaxDistanceSqr[button] >=
