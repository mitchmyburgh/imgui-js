...
export var ImguiRenderMixin = {
  ...
  renderText(pos, text, hide_text_after_hash = true) {
    ...
    if (hide_text_after_hash) text = text.split("##")[0];
    ...
    if (g.LogEnabled) this.logRenderedText(pos, text);
  },
  ...
  renderTextClippedEx(
    ...
    let g = this.guictx;
    let pos = pos_min;
    let text_size = text_size_if_known
      ? text_size_if_known
    ...
    // Render
    if (need_clipping) {
      let fine_clip_rect = Rect.FromXY(
        clip_min.x,
        clip_min.y,
        clip_max.x,
        clip_max.y
  ...
  renderFrame(p_min, p_max, fill_col, border = true, rounding = 0) {
    ...
    if (border) this.renderFrameBorder(p_min, p_max, rounding);
  },
  ...
  renderColorRectWithAlphaCheckerboard(
    ...
    if (on.Equals(off)) {
      ...
      for (let y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++) {
        ...
        for (
          let x = p_min.x + grid_off.x + (yi & 1) * grid_step;
          x < p_max.x;
          x += grid_step * 2
        ) {
  ...
  renderNavHighlight(bb, id, flags = NavHighlightFlags.TypeDefault) {
    ...
    let rounding =
      flags & NavHighlightFlags.NoRounding ? 0 : g.Style.FrameRounding;
    ...
    if (flags & NavHighlightFlags.TypeDefault) {
      ...
      window.DrawList.AddRect(
        Vec2.Add(display_rect.Min, thickness * 0.5),
        Vec2.Subtract(display_rect.Max, thickness * 0.5),
  ...
  renderRectFilledRangeH(
    ...
    let p0 = new Vec2(
      Vec1.Lerp(rect.Min.x, rect.Max.x, x_start_norm),
      rect.Min.y
    );
    let p1 = new Vec2(
