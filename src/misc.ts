...
export var ImguiMiscMixin = {
  ...
  startMouseMovingWindow(win) {
    ...
    if (
      win.Flags & WindowFlags.NoMove ||
      win.RootWindow.Flags & WindowFlags.NoMove
    ) {
      can_move_window = false;
    }
    if (can_move_window) g.MovingWindow = win;
  },
  ...
  updateMouseMovingWindowEndFrame() {
    ...
    if (g.IO.MouseClicked[0]) {
      ...
      } else if (g.NavWindow && !this.getFrontMostPopupModal()) {
  ...
  setFocusID(id, win) {
    ...
    if (g.NavWindow != win) g.NavInitRequest = false;
    g.NavId = id;
    g.NavWindow = win;
  ...
  itemHoverable(bb, id) {
    ...
    if (g.HoveredWindow != win) return false;
    if (g.ActiveId != 0 && g.ActiveId != id && !g.ActiveIdAllowOverlap)
      return false;
    if (!this.IsMouseHoveringRect(bb.Min, bb.Max)) return false;
    if (
    ...
    if (win.DC.ItemFlags & ItemFlags.Disabled) return false;
  ...
  focusableItemRegister(win, id) {
    ...
    if (
      ...
      g.FocusRequestNextCounterTab =
        win.DC.FocusCounterTab + (g.IO.KeyShift ? (is_tab_stop ? -1 : 0) : +1);
  ...
  updateMouseWheel() {
    ...
    while (
      scroll_window.Flags & WindowFlags.ChildWindow &&
    ...
    if (g.IO.MouseWheel != 0 || g.IO.TouchDelta.y != 0) {
      if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling) {
        ...
        if (new_font_scale < 0.5) new_font_scale = 0.5;
    ...
    if (
      g.IO.MouseWheelH != 0 ||
      (g.IO.TouchDelta.x != 0 && scroll_allowed && !g.IO.KeyCtrl)
    ) {
