...
export var ImguiWinMgrMixin = {
  /**
    ...
    //   button is pressed.
     */
  Begin(name: string, p_open = null, flags = 0) {
    let g = this.guictx;
    const style = g.Style;
    ...
    if (window_just_created) {
      ...
      let sz =
        g.NextWindowData.SizeCond != 0 ? g.NextWindowData.SizeVal : Vec2.Zero();
    ...
    let window_size_x_set_by_api = false,
      window_size_y_set_by_api = false;
    if (g.NextWindowData.PosCond) {
      window_pos_set_by_api =
        (win.SetWindowPosAllowFlags & g.NextWindowData.PosCond) != 0;
      if (
        window_pos_set_by_api &&
    ...
    if (g.NextWindowData.SizeCond) {
      window_size_x_set_by_api =
        (win.SetWindowSizeAllowFlags & g.NextWindowData.SizeCond) != 0 &&
    ...
    if (g.NextWindowData.CollapsedCond)
      win.SetWindowCollapsed(
        g.NextWindowData.CollapsedVal,
        g.NextWindowData.CollapsedCond
      );
    if (g.NextWindowData.FocusCond) this.FocusWindow(win);
    if (win.Appearing)
    ...
    if (first_begin_of_the_frame) {
      // Initialize
      const window_is_child_tooltip =
        flags & WindowFlags.ChildWindow && flags & WindowFlags.Tooltip;
      ...
      if (
        window_title_visible_elsewhere &&
        !window_just_created &&
        name != win.Name
      ) {
        win.Name = name;
      ...
      if (win.HiddenFramesCanSkipItems > 0) win.HiddenFramesCanSkipItems--;
      ...
      if (
        ...
        if (flags & WindowFlags.AlwaysAutoResize) {
          if (!window_size_x_set_by_api) win.Size.x = win.SizeFull.x = 0;
          if (!window_size_y_set_by_api) win.Size.y = win.SizeFull.y = 0;
      ...
      if (
        ...
        win.WindowPadding = new Vec2(
          0,
          flags & WindowFlags.MenuBar ? style.WindowPadding.y : 0
        );
      }
      win.DC.MenuBarOffset.x = Math.max(
        Math.max(win.WindowPadding.x, style.ItemSpacing.x),
      ...
      if (flags & WindowFlags.AlwaysAutoResize && !win.Collapsed) {
        ...
        if (!window_size_x_set_by_api && win.AutoFitFramesX > 0) {
          win.SizeFull.x = size_full_modified.x = win.AutoFitOnlyGrows
            ? Math.max(win.SizeFull.x, size_auto_fit.x)
            : size_auto_fit.x;
        }
        if (!window_size_y_set_by_api && win.AutoFitFramesY > 0) {
          win.SizeFull.y = size_full_modified.y = win.AutoFitOnlyGrows
            ? Math.max(win.SizeFull.y, size_auto_fit.y)
            : size_auto_fit.y;
        }
        if (!win.Collapsed) {
          this.MarkIniSettingsDirty(win);
      ...
      win.Size =
        win.Collapsed && !(flags & WindowFlags.ChildWindow)
          ? win.TitleBarRect().GetSize()
          : win.SizeFull;
      ...
      if (!win.Collapsed) {
        ...
        if (win.ScrollbarX && !win.ScrollbarY) {
          win.ScrollbarY =
            win.SizeContents.y > size_y_for_scrollbars - style.ScrollbarSize &&
            !(flags & WindowFlags.NoScrollbar);
        }
        win.ScrollbarSizes = new Vec2(
          win.ScrollbarY ? style.ScrollbarSize : 0,
      ...
      if (window_pos_with_pivot) {
        ...
        (flags & WindowFlags.Popup) != 0 &&
        !window_pos_set_by_api &&
        window_just_appearing_after_hidden_for_resize
      ) {
        win.Pos = this.findBestWindowPosForPopup(win);
      } else if (
        (flags & WindowFlags.Tooltip) != 0 &&
        !window_pos_set_by_api &&
        !window_is_child_tooltip
      ) {
        win.Pos = this.findBestWindowPosForPopup(win);
      ...
      win.WindowRounding =
        flags & WindowFlags.ChildWindow
          ? style.ChildRounding
          : flags & WindowFlags.Popup && !(flags & WindowFlags.Modal)
          ? style.PopupRounding
      ...
      const grip_draw_size = Math.floor(
        Math.max(g.FontSize * 1.35, win.WindowRounding + 1 + g.FontSize * 0.2)
      );
      if (!win.Collapsed) {
        win.UpdateManualResize(
          size_auto_fit,
          border_held,
          resize_grip_count,
          resize_grip_col
      ...
      } else win.ItemWidthDefault = Math.floor(g.FontSize * 16);
      ...
      win.DrawList.Clear();
      win.DrawList.Flags =
        (g.Style.AntiAliasedLines ? DrawListFlags.AntiAliasedLines : 0) |
        (g.Style.AntiAliasedFill ? DrawListFlags.AntiAliasedFill : 0);
      // win.DrawList.PushTextureID(g.Font.ContainerAtlas.TexID);
      if (flags & WindowFlags.Tooltip)
        // dbadb.. native does this in flatting drawlists
      ...
      if (win.Collapsed) {
        ...
        if (!(flags & WindowFlags.NoBackground)) {
          ...
          win.DrawList.AddRectFilled(
            ...
            bg_col,
            window_rounding,
            flags & WindowFlags.NoTitleBar ? CornerFlags.All : CornerFlags.Bot
          );
        }
        ...
        if (!(flags & WindowFlags.NoTitleBar)) {
          let title_bar_col = style.GetColor(
            title_bar_is_highlight ? "TitleBgActive" : "TitleBg"
          );
          win.DrawList.AddRectFilled(
            title_bar_rect.Min,
            title_bar_rect.Max,
        ...
        if (flags & WindowFlags.MenuBar) {
          ...
          win.DrawList.AddRectFilled(
            Vec2.AddXY(menu_bar_rect.Min, window_border_size, 0),
      ...
      // relies on it.
      win.ContentsRegionRect.Min.x =
        win.Pos.x - win.Scroll.x + win.WindowPadding.x;
      win.ContentsRegionRect.Min.y =
        win.Pos.y -
        win.Scroll.y +
        win.WindowPadding.y +
        win.TitleBarHeight() +
        win.MenuBarHeight();
      win.ContentsRegionRect.Max.x =
        win.Pos.x -
        win.Scroll.x -
        win.WindowPadding.x +
      ...
      win.DC.CursorStartPos = Vec2.AddXY(
        win.Pos,
        win.DC.Indent.x + win.DC.ColumnsOffset.x,
        win.TitleBarHeight() +
          win.MenuBarHeight() +
          win.WindowPadding.y -
          win.Scroll.y
      ...
      if (!(flags & WindowFlags.NoTitleBar)) {
        ...
        if (p_open != null) {
          ...
          if (
            this.CloseButton(
              win.GetID("#CLOSE"),
              new Vec2(
                win.Pos.x + win.Size.x - style.FramePadding.x - rad,
        ...
        let marker_size_x =
          flags & WindowFlags.UnsavedDocument
            ? this.CalcTextSize(UNSAVED_DOCUMENT_MARKER, false).x
            : 0;
        let text_size = Vec2.AddXY(
          this.CalcTextSize(title, true),
          marker_size_x,
          0
        );
        ...
        if (flags & WindowFlags.UnsavedDocument) {
          ...
          let marker_pos = Vec2.AddXY(
            new Vec2(x, text_r.Min.y),
            2 - marker_size_x,
            0
          );
          let off = new Vec2(0, Math.floor(-g.FontSize * 0.25));
          this.renderTextClipped(
            Vec2.Add(marker_pos, off),
      ...
      win.InnerMainRect.Min.y =
        title_bar_rect.Max.y +
        win.MenuBarHeight() +
        (flags & WindowFlags.MenuBar || !(flags & WindowFlags.NoTitleBar)
          ? style.FrameBorderSize
          : win.WindowBorderSize);
      win.InnerMainRect.Max.x =
        win.Pos.x + win.Size.x - win.ScrollbarSizes.x - win.WindowBorderSize;
      win.InnerMainRect.Max.y =
        win.Pos.y + win.Size.y - win.ScrollbarSizes.y - win.WindowBorderSize;
      //win.DrawList.AddRect(win.InnerRect.Min, win.InnerRect.Max, IM_COL32_WHITE);

      // Inner clipping rectangle
      // Force round operator last to ensure that e.g. (int)(max.x-min.x) in user's render code produce correct result.
      win.InnerClipRect.Min.x = Math.floor(
        0.5 +
          win.InnerMainRect.Min.x +
          Math.max(
            0,
            Math.floor(win.WindowPadding.x * 0.5 - win.WindowBorderSize)
          )
      );
      win.InnerClipRect.Min.y = Math.floor(0.5 + win.InnerMainRect.Min.y);
      win.InnerClipRect.Max.x = Math.floor(
        0.5 +
          win.InnerMainRect.Max.x -
          Math.max(
            0,
      ...
      win.DC.LastItemStatusFlags = this.IsMouseHoveringRect(
        title_bar_rect.Min,
        title_bar_rect.Max,
        false
      )
        ? ItemStatusFlags.HoveredRect
        : 0;
      win.DC.LastItemRect = title_bar_rect;
    } // not first begin of frame
    else {
      // Append
      this.setCurrentWindow(win);
    }
    ...
    // window is unused)
    if (first_begin_of_the_frame) win.WriteAccessed = false;
    ...
    if (flags & WindowFlags.ChildWindow) {
      ...
      if (
        ...
        if (
          win.OuterRectClipped.Min.x >= win.OuterRectClipped.Max.x ||
    ...
    if (style.Alpha <= 0) win.HiddenFramesCanSkipItems = 1;

    // Update the Hidden flag
    win.Hidden =
      win.HiddenFramesCanSkipItems > 0 || win.HiddenFramesCannotSkipItems > 0;
  ...
  End() {
    ...
    if (win.DC.CurrentColumns != null) this.endColumns();
    ...
    if (win.Flags & WindowFlags.Popup) g.BeginPopupStack.pop_back();
  ...
  beginChildEx(name, id, size_arg, border, flags) {
    ...
    let title;
    if (name) title = `${parent_window.Name}/${name}_${id & 0xfffff}`;
    else title = `${parent_window.Name}/${id & 0xfffff}`;

    const backup_border_size = g.Style.ChildBorderSize;
    if (!border) g.Style.ChildBorderSize = 0;
  ...
  EndChild() {
    ...
    console.assert(
      ...
      "Mismatched BeginChild()/EndChild()"
    );
    if (win.BeginCount > 1) {
      this.End();
    } else {
      ...
      // than a 0
      if (win.AutoFitChildAxises & (1 << Axis.X)) sz.x = Math.max(4, sz.x);
      if (win.AutoFitChildAxises & (1 << Axis.Y)) sz.y = Math.max(4, sz.y);
      this.End();
      ...
      this.itemSize(sz);
      if (
        (win.DC.NavLayerActiveMask != 0 || win.DC.NavHasScroll) &&
  ...
  BeginChildFrame(id, size, extra_flags = 0) {
    ...
    let ret = this.BeginChild(
      id,
      size,
      true,
      WindowFlags.NoMove | WindowFlags.AlwaysUseWindowPadding | extra_flags
    );
    this.PopStyleVar(3);
    this.PopStyleColor();
    return ret;
  ...
  IsWindowFocused(flags) {
    let g = this.guictx;
    if (flags & FocusedFlags.AnyWindow) return g.NavWindow != null;
  ...
  IsWindowHovered(flags) {
    ...
    if (!this.isWindowContentHoverable(g.HoveredWindow, flags)) return false;
  ...
  SetWindowPosNm(nm, pos, cond) {
    ...
    if (win) win.SetWindowPos(pos, cond);
  },
  ...
  SetWindowSizeNm(nm, size, cond) {
    let win = this.findWindowByName(nm);
    if (win) win.SetWindowSize(size, cond);
  },
  ...
  SetWindowCollapsedNm(nm, collapsed, cond) {
    let win = this.findWindowByName(nm);
    if (win) win.SetWindowCollapsed(collapsed, cond);
  },
  ...
  SetScrollHereY(center_y_ratio = 0.5) {
    ...
    target_y +=
      win.DC.PrevLineHeight * center_y_ratio +
  ...
  createNewWindow(name, size, flags) {
    ...
    if ((flags & WindowFlags.AlwaysAutoResize) != 0) {
      ...
      if (win.Size.x <= 0) win.AutoFitFramesX = 2;
      if (win.Size.y <= 0) win.AutoFitFramesY = 2;
    ...
    else g.Windows.push_back(win);
    return win;
  },

  setCurrentWindow(win) {
    this.guictx.CurrentWindow = win;
    if (win) win.MakeCurrent();
  },
  ...
  getWindowBgColorFromFlags(flags) {
    if (flags & (WindowFlags.Tooltip | WindowFlags.Popup)) return "PopupBg";
    if (flags & WindowFlags.ChildWindow) return "ChildBg";
    return "WindowBg";
  },
  ...
  FocusWindow(win) {
    ...
    if (g.NavWindow != win) {
      g.NavWindow = win;
      if (win && g.NavDisableMouseHover) g.NavMousePosDirty = true;
    ...
    if (win.RootWindow) win = win.RootWindow;
  ...
  focusPreviousWindowIgnoringOne(ignorewin) {
    ...
    for (let i = g.WindowsFocusOrder.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        if (
          (win.Flags & (WindowFlags.NoMouseInputs | WindowFlags.NoNavInputs)) !=
  ...
  getWindowScrollMaxX(win) {
    return Math.max(
      0,
      win.SizeContents.x - (win.SizeFull.x - win.ScrollbarSizes.x)
    );
  },
  ...
  getWindowScrollMaxY(win) {
    return Math.max(
      0,
      win.SizeContents.y - (win.SizeFull.y - win.ScrollbarSizes.y)
    );
  },
  ...
  getWindowAllowedExtentRect(win_unused) {
    ...
    r_screen.expandXY(
      r_screen.GetWidth() > padding.x * 2 ? -padding.x : 0,
  ...
  updateHoveredWindowAndCaptureFlags() {
    ...
    if (g.WantCaptureMouseNextFrame != -1)
      ...
      g.IO.WantCaptureMouse =
        (mouseAvail && (g.HoveredWindow != null || anyDown)) ||
  ...
  findHoveredWindow() {
    ...
    for (let i = g.Windows.length - 1; i >= 0; i--) {
      ...
      if (win.Flags & WindowFlags.NoMouseInputs) continue;
  ...
  getFrontMostPopupModal() {
    ...
    for (let n = g.OpenPopupStack.length - 1; n >= 0; n--) {
      ...
      if (popup && popup.Flags & WindowFlags.Modal) return popup;
    }
    return null;
  },
  ...
  _zsort(a, b) {
    ...
    if (
      a.Active &&
      !(a.Flags & WindowFlags.ChildWindow) &&
      b.Active &&
      !(b.Flags & WindowFlags.ChildWindow)
    ) {
  ...
  addWindowToSortBuffer(buf, win) {
    ...
    if (win.Active) {
      ...
      if (count > 1) win.DC.ChildWindows.sort(this._compareWindows);
      for (let i = 0; i < count; i++) {
        let c = win.DC.ChildWindows[i];
        if (c.Active) this.addWindowToSortBuffer(buf, c);
      }
    }
  },
};
