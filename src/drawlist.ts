...
export class DrawList {
  ...
  Render(
    ...
    if (layer == undefined) {
      ...
      if (drawlayer != undefined) {
        for (let i = 0; i < drawlayer.length; i++) drawlayer[i]();
      }
    }
    this.Clear(layer);
  }
  ...
  AddLine(a, b, col, thickness = 1) {
    if (typeof col != "string") col = col.AsStr();
  ...
  AddLines(lines, thickness) {
    this.currentLayer.push(
      this.drawLines.bind(this, lines, thickness, this.getClipRect())
    );
  }
  ...
  AddRectRamp(
    ...
    b,
    ramp,
    ttob // ttob ? top-to-bottom : left-to-right
  ) {
    let w = b.x - a.x;
    let h = b.y - a.y;
    // XXX: do we need to "pickle" ramp?
    this.currentLayer.push(
      this.drawRectRamp.bind(
        this,
        a.x,
        a.y,
        w,
  ...
  AddRectsFilled(fill, rects, rounding = 0) {
    this.currentLayer.push(
      this.drawRectsFilled.bind(this, fill, rects, rounding, this.getClipRect())
    );
  }
  ...
  AddRectFilledMultiColor(a, b, uleftCol, urightCol, brightCol, bleftCol) {
    console.assert(
      uleftCol.Equals(urightCol) || uleftCol.Equals(bleftCol),
  ...
  AddTexts(attrs, tarray) {
    let clip = this.getClipRect();
    this.currentLayer.push(this.fillTexts.bind(this, attrs, tarray, clip));
  }
  ...
  PathFillConvex(col) {
    if (typeof col != "string") col = col.AsStr();
    this.currentLayer.push(
      this.drawPath.bind(this, this.path, col, "fillConvex")
    );
    this.path = [];
  }
  ...
  PathStroke(col, closed, thickness = 1) {
    if (typeof col != "string") col = col.AsStr();
    this.currentLayer.push(
      this.drawPath.bind(this, this.path, col, "stroke", closed, thickness)
    );
    this.path = [];
  }
  ...
  drawRectRamp(x, y, w, h, stops, ttob, clip) {
    ...
    if (clip) this.doClip(ctx, clip);
    let grd;
    if (ttob) grd = ctx.createLinearGradient(x, y, x, y + h);
    else grd = ctx.createLinearGradient(x, y, x + w, y);
    let t = 0,
      dt = 1 / (stops.length - 1);
    for (let c of stops) {
        ...
        case PathType.Bezier:
          {
            ...
            if (i != 0) ctx.bezierCurveTo(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y);
          }
          break;
        case PathType.Rect:
          {
  ...
  fillText(txt, x, y, fillStyle, baseline, align, font = null, clip = null) {
    ...
    if (clip) this.doClip(ctx, clip);
    if (font) {
      if (font.AsStr) ctx.font = font.AsStr();
      else ctx.font = font;
    }
