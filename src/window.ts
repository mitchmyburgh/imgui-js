...
const WindowResizeFromEdgeTimer = 0.04; // was WindowResizeFromEdgeTimer
...
export const ResizeGripDefs = [
  new ResizeGripDef(new Vec2(1, 1), new Vec2(-1, -1), 0, 3), // Lower right
  new ResizeGripDef(new Vec2(0, 1), new Vec2(+1, -1), 3, 6), // Lower left
  new ResizeGripDef(new Vec2(0, 0), new Vec2(+1, +1), 6, 9), // Upper left
  new ResizeGripDef(new Vec2(1, 0), new Vec2(-1, +1), 9, 12), // Upper right
];
...
export class Window {
  ...
  GetIDFromRectangle(r_abs) {
    const seed = this.IDStack.back();
    const r_rel = [
      Math.floor(r_abs.Min.x - this.Pos.x),
  ...
  MakeCurrent() {
    ...
    if (this.Font) this.imgui.SetFont(this.Font);
    else this.imgui.SetFont(this.imgui.GetFont(null, this.FontWindowScale));
  }

  Render(
    layer // layer undefined: draw layers 0, 1
  ) {
  ...
  TitleBarHeight() {
    if (this.Flags & WindowFlags.NoTitleBar) return 0;
    else
  ...
  UpdateWindowParentAndRootLinks(flags, parent) {
    ...
    this.RootWindow =
      this.RootWindowForTitleBarHighlight =
  ...
  GetScrollMaxX() {
    return Math.max(
      0,
      this.SizeContents.x - (this.SizeFull.x - this.ScrollbarSizes.x)
    );
  }
  ...
  GetScrollMaxY() {
    return Math.max(
      0,
      this.SizeContents.y - (this.SizeFull.y - this.ScrollbarSizes.y)
    );
  }
  ...
  CalcSizeAutoFit(size_contents) {
    ...
    if (this.Flags & WindowFlags.Tooltip) {
      ...
      if (is_popup || is_menu) size_min = Vec2.Min(size_min, new Vec2(4, 4));
      let size_max = Vec2.Max(
        size_min,
        Vec2.Subtract(
          g.IO.DisplaySize,
  ...
  CalcSizeAfterConstraint(new_size) {
    ...
    if (g.NextWindowData.SizeConstraintCond != 0) {
      ...
      new_size.x =
        cr.Min.x >= 0 && cr.Max.x >= 0
          ? Vec1.Clamp(new_size.x, cr.Min.x, cr.Max.x)
          : this.SizeFull.x;
      new_size.y =
        cr.Min.y >= 0 && cr.Max.y >= 0
    ...
    // Minimum size
    if (
      !(this.Flags & (WindowFlags.ChildWindow | WindowFlags.AlwaysAutoResize))
    ) {
  ...
  getBgColor(flag, alpha) {
    let key;
    if (flag & (WindowFlags.Tooltip | WindowFlags.Popup)) key = "PopupBg";
    else if (flag & WindowFlags.ChildWindow) key = "ChildBg";
    else key = "WindowBg";
  ...
  CalcNextScrollFromScrollTargetAndClamp(snap_on_edges) {
    ...
    if (this.ScrollTarget.x < Number.MAX_VALUE) {
      ...
      scroll.x =
        this.ScrollTarget.x - cr_x * (this.SizeFull.x - this.ScrollbarSizes.x);
    }
    if (this.ScrollTarget.y < Number.MAX_VALUE) {
      ...
      if (
        ...
        cr_y >= 1 &&
        target_y >=
          this.SizeContents.y - this.WindowPadding.y + g.Style.ItemSpacing.y
      ) {
        target_y = this.SizeContents.y;
      }
      scroll.y =
        target_y -
        (1 - cr_y) * (this.TitleBarHeight() + this.MenuBarHeight()) -
  ...
  ClampWindowRect(rect, padding) {
    ...
    this.Pos = Vec2.Min(
      ...
      Vec2.Subtract(
        Vec2.Max(Vec2.Add(this.Pos, sz), Vec2.Add(rect.Min, padding)),
        sz
      )
    );
  }
  ...
  UpdateManualResize(autofit, borderheld, resize_grip_count, resize_grip_col) {
    ...
    const grip_draw_size = Math.floor(
      Math.max(g.FontSize * 1.35, this.WindowRounding + 1 + g.FontSize * 0.2)
    );
    ...
    for (
      ...
      let resize_rect = new Rect(
        Vec2.Subtract(corner, Vec2.Mult(grip.InnerDir, grip_hover_outersize)),
        Vec2.Add(corner, Vec2.Mult(grip.InnerDir, grip_hover_innersize))
      );
      ...
      if (held.get() && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
        ...
        this.calcResizePosSizeFromAnyCorner(
          corner_target,
          grip.CornerPosN,
          pos_target /*output*/,
          size_target /*output*/
    ...
    for (let border_n = 0; border_n < resize_border_count; border_n++) {
      let border_rect = this.getResizeBorderRect(
        border_n,
      ...
      imgui.ButtonBehavior(
        border_rect,
        this.GetID(border_n + 4),
        hovered,
        held,
        ButtonFlags.FlattenChildren
      );
      if (
        (hovered.get() && g.HoveredIdTimer > WindowResizeFromEdgeTimer) ||
        held.get()
      ) {
        g.MouseCursor =
          border_n & 1 ? MouseCursor.ResizeEW : MouseCursor.ResizeNS;
        if (held.get()) borderheld.set(border_n);
      }
      if (held.get()) {
  ...
  SetWindowConditionAllowFlags(flags, enabled) {
    if (enabled) {
      ...
      this.SetWindowCollapsedAllowFlags =
        this.SetWindowCollapsedAllowFlags | flags;
    } else {
      this.SetWindowPosAllowFlags = this.SetWindowPosAllowFlags & ~flags;
      this.SetWindowSizeAllowFlags = this.SetWindowSizeAllowFlags & ~flags;
      this.SetWindowCollapsedAllowFlags =
  ...
  RenderOuterBorders() {
    ...
    if (border_held != -1) {
      ...
      this.DrawList.PathArcTo(
        Vec2.Add(Vec2.Add(p0, half), Vec2.Mult(def.InnerDir, rounding)),
        rounding,
        def.OuterAngle - Math.PI * 0.25,
        def.OuterAngle
      );
      ...
      this.DrawList.PathStroke(
        imgui.GetStyleColor("SeparatorActive"),
        false,
        Math.max(2.0, border_size)
      ); // Thicker than usual
    }
    if (g.Style.FrameBorderSize > 0 && !(this.Flags & WindowFlags.NoTitleBar)) {
...
export class GroupData {
  constructor(win, guictx) {
    ...
    this.BackupActiveIdPreviousFrameIsAlive =
      guictx.ActiveIdPreviousFrameIsAlive;
