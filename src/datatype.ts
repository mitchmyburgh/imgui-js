...
const GDataTypeInfo = [
  ...
  [4, "%f", [-Number.MAX_VALUE * 0.5, Number.Max_VALUE * 0.5]],
  [8, "%f", [-Number.MAX_VALUE * 0.5, Number.Max_VALUE * 0.5]],
];
...
export function FormatValue(v, format, precision = 0) {
  ...
  if (typeof v != "number") {
    ...
    vstr = v;
  } else vstr = v.toFixed(precision);
  if (digits) vstr = ("000" + vstr).slice(-digits);
  if (j) return format.slice(0, i) + vstr + format.slice(j + 1);
  else return vstr;
}
...
export function DataTypeApplyOpFromText(
  buf,
  buforig,
  data_type,
  format,
...
export function GetMinStepAtFloatPrecision(precision) {
  if (precision < 0) return Number.MIN_VALUE;
  return precision < MinStep.length
    ? MinStep[precision]
