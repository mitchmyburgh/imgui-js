...
export class Color {
  ...
  static FromCSS(cstr: string) {
    ...
    if (m) {
      ...
      let parts = base.split(size <= 4 ? /(.)/ : /(..)/).filter((x) => x != "");
      fields = parts.map(function (x) {
        if (size <= 4) return parseInt(x + "0", 16) / 255;
        else return parseInt(x, 16) / 255;
      });
    } else return null;
  ...
  AsFloatStr(noalpha: boolean) {
    ...
    if (noalpha) return `${this.space}(${x}, ${y}, ${z})`;
  ...
  AsHashStr(
    noalpha = false // hash strings always in RGB
  ) {
    let c = this.AsRGB();
    ...
    if (noalpha) return `#${r}${g}${b}`.toUpperCase();
    else {
  ...
  AsHexStr(
    noalpha = false // hex strings always in rgb (currently)
  ) {
    let c = this.AsRGB();
    let r = ("0" + Math.floor(c.x * 255).toString(16)).slice(-2);
    let g = ("0" + Math.floor(c.y * 255).toString(16)).slice(-2);
    let b = ("0" + Math.floor(c.z * 255).toString(16)).slice(-2);
    if (noalpha) return `0x${r}${g}${b}`;
    else {
      let a = ("0" + Math.floor(c.a * 255).toString(16)).slice(-2);
      return `0x${r}${g}${b}${a}`;
    }
...
Colors = {
  ...
  dark1: Color.rgb(0.05, 0.05, 0.08),
  dark2: Color.rgb(0.08, 0.08, 0.1),
  dark3: Color.rgb(0.1, 0.1, 0.15),
  mid1: Color.rgb(0.4, 0.4, 0.4),
};
